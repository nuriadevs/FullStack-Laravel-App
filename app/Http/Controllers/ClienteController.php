        ...
        try {
            ...
            if ($search) {
                if (!preg_match('/^[a-zA-Z\s]+$/', $search)) {
                    return redirect()->route('usuarios.cliente.index')->with('error', 'El formato del nombre es incorrecto. Solo se permiten letras y espacios.');
                }
                ...
                // Verificar si no se encontraron clientes con el nombre buscado
                $mensaje = $clientes->isEmpty() ? "No se encontraron clientes para el nombre '$search'." : null;
            } else {
