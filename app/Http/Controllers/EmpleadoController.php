        ...
        try {
            ...
            if ($search) {
                if (!preg_match('/^[a-zA-Z\s]+$/', $search)) {
                    return redirect()->route('usuarios.empleado.index')->with('error', 'El formato del nombre es incorrecto. Solo se permiten letras y espacios.');
                }
                // Buscar los empleados por su nombre
                $empleados = Empleado::whereHas('user', function ($query) use ($search) {
                    $query->where('name', 'LIKE', "%$search%");
                })->paginate(10);
    
                // Verificar si no se encontraron empleados con el nombre buscado
                $mensaje = $empleados->isEmpty() ? "No se encontraron empleados para el nombre '$search'." : null;
            } else {
