    ...
    public function store(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'cp' => 'required|digits:5|regex:/^[0-9][0-9]+$/',
        ...
        try {
            ...
            if ($search) {
                if (!preg_match('/^[a-zA-Z\s]+$/', $search)) {
                    return redirect()->route('usuarios.administrador.index')->with('error', 'El formato del nombre es incorrecto. Solo se permiten letras y espacios.');
                }
                // Buscar los administradores por su nombre
                $administradores = User::whereHas('perfil', function ($query) {
                    $query->where('tipo', 'administrador');
                })->where('name', 'LIKE', "%$search%")->paginate(10);

                // Verificar si no se encontraron administradores con el nombre buscado
                $mensaje = $administradores->isEmpty() ? "No se encontraron administradores para el nombre '$search'." : null;
            } else {
