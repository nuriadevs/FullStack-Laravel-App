        ...
        try {
            ...
            if ($search) {
                ...
                if (!preg_match('/^[a-zA-Z\s]+$/', $search)) {
                    return redirect()->route('facturas.index')->with('error', 'El formato del nombre es incorrecto. Solo se permiten letras y espacios.');
                }
                ...
                if (!$cliente) {
                    return redirect()->route('facturas.index')->with('error', "No se encontró ningún cliente con el nombre '$search'.");
                }

                // Buscar las facturas del cliente
                $facturas = Factura::where('cliente_id', $cliente->id)->paginate(10);

                // Verificar si no se encontraron facturas para el cliente
                $mensaje = $facturas->isEmpty() ? "No se encontraron facturas para el cliente con el nombre '$search'." : null;
            } else {
                // No se proporcionó un término de búsqueda, mostrar todas las facturas
