...
class CitaController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            ...
            // Normalizar los nombres para ignorar tildes y hacer insensible a mayúsculas y minúsculas
            $nombreClienteDB = Str::lower(Str::ascii($cliente->name));
            $nombreClienteRequest = Str::lower(Str::ascii($request->nombre_cliente));

            // Verificar que el nombre proporcionado coincide con el cliente que tiene el DNI proporcionado
            if ($nombreClienteDB !== $nombreClienteRequest) {
    ...
    public function buscar(Request $request)
    {
        try {
            ...
            if ($search) {
                // Verificar si el valor introducido es un nombre válido
                if (!preg_match('/^[a-zA-Z\s]+$/', $search)) {
                    return redirect()->route('citas.index')->with('error', 'El formato del nombre es incorrecto. Solo se permiten letras y espacios.');
                }
                ...
                if (!$cliente) {
                    return redirect()->route('citas.index')->with('error', "No se encontró ningún cliente con el nombre '$search'.");
                }
    
                // Buscar las citas del cliente
                $citas = Cita::where('cliente_id', $cliente->id)->paginate(10);
    
                // Verificar si no se encontraron citas para el cliente
                $mensaje = $citas->isEmpty() ? "No se encontraron citas para el cliente con el nombre '$search'." : null;
            } else {
                // No se proporcionó un término de búsqueda, mostrar todas las citas
                $citas = Cita::paginate(10);
                $mensaje = null;
            }
